@extends('layouts.admin', ['title' => 'New Game'])
...
<div style="max-width:1100px;">
    ...
    <div style="margin-bottom:48px;">
        ...
        <h1 style="font-size:2.8rem; font-weight:900; letter-spacing:-0.05em; background:linear-gradient(to right, #fff, var(--accent-light)); -webkit-background-clip:text; -webkit-text-fill-color:transparent;">Add New Game</h1>
    </div>

    @if ($errors->any())
        <div style="background:rgba(244, 63, 94, 0.1); border:1px solid rgba(244, 63, 94, 0.2); padding:24px; border-radius:24px; margin-bottom:40px; color:#fb7185; font-size:14px; animation: shake 0.5s ease-in-out;">
    ...
    <form action="{{ route('admin.games.store') }}" method="POST" enctype="multipart/form-data" style="display:flex; flex-direction:column; gap:40px;">
        @csrf
        ...
        <div style="display:grid; grid-template-columns:1.6fr 1fr; gap:40px; align-items:start;">
            ...
            <div class="card" style="padding:40px; border-radius:32px;">
                ...
                <div style="display:flex; flex-direction:column; gap:32px;">
                    <div class="form-group">
                        ...
                        <input type="text" name="title" value="{{ old('title') }}" class="form-input" required placeholder="e.g. VALORANT" style="padding:16px 24px; border-radius:16px;">
                    </div>
                    
                    <div class="form-group">
                        <label class="form-label" style="font-weight:800; margin-bottom:12px;">Description</label>
                        <textarea name="description" class="form-input" rows="8" placeholder="Provide a detailed overview of mechanics, competitive nature, and platform..." style="padding:16px 24px; border-radius:16px;">{{ old('description') }}</textarea>
                    </div>
                </div>
            </div>
            ...
            <div class="card" style="padding:40px; border-radius:32px;">
                ...
                <div x-data="{ preview: null }" style="display:flex; flex-direction:column; gap:28px;">
                    <div style="aspect-ratio:16/9; background:#000; border:2px dashed var(--border); border-radius:24px; overflow:hidden; display:flex; align-items:center; justify-content:center; position:relative; transition:0.3s;" :style="preview ? 'border-style:solid' : ''">
                        ...
                        <template x-if="!preview">
                            <div style="text-align:center; color:var(--text-3); padding:40px;">
                                ...
                                <p style="font-size:13px; font-weight:800; text-transform:uppercase; letter-spacing:0.1em;">Awaiting Artwork</p>
                            </div>
                        </template>
                    </div>
                    
                    <div style="position:relative;">
                        <input type="file" name="cover" @change="const file = $event.target.files[0]; if(file) { const reader = new FileReader(); reader.onload = (e) => preview = e.target.result; reader.readAsDataURL(file); }" class="form-input" required style="width:100%; padding:14px; cursor:pointer; border-radius:16px;">
                    </div>
                    <p style="font-size:13px; color:var(--text-3); line-height:1.6;">Recommended: 1920x1080px. Max 2MB.</p>
                </div>
            </div>
        </div>

        {{-- Ranks --}}
        <div x-data="{ ranks: [] }" class="card" style="padding:40px; border-radius:32px;">
            <div style="display:flex; align-items:center; justify-content:space-between; margin-bottom:40px;">
                <div>
                    <h3 style="font-weight:900; font-size:1.6rem; margin-bottom:8px;">Competitive Ecosystem</h3>
                    <p style="font-size:15px; color:var(--text-2);">Define the ranking tiers that players can select for this game.</p>
                </div>
            ...
            <div style="display:grid; grid-template-columns:repeat(auto-fill, minmax(320px, 1fr)); gap:24px;">
                <template x-for="(rank, index) in ranks" :key="index">
                    <div style="display:flex; flex-direction:column; gap:20px; padding:28px; background:rgba(0,0,0,0.25); border:1px solid var(--border); border-radius:28px; position:relative; animation: slideIn 0.3s ease-out;">
                        <div class="form-group">
                            <label class="form-label" style="font-weight:800; font-size:13px; text-transform:uppercase; letter-spacing:0.05em;">Tier Name</label>
                            <input type="text" :name="'ranks['+index+'][title]'" x-model="rank.title" class="form-input" required placeholder="e.g. Diamond" style="padding:14px 20px; border-radius:14px;">
                        </div>
                        <div class="form-group">
                            <label class="form-label" style="font-weight:800; font-size:13px; text-transform:uppercase; letter-spacing:0.05em;">Progression Level</label>
                        ...
                        <div class="form-group">
                            ...
                            <input type="file" :name="'ranks['+index+'][icon]'" class="form-input" style="padding:10px 14px; border-radius:12px; font-size:12px;">
                        </div>
                ...
                <template x-if="ranks.length === 0">
                    <div style="grid-column: 1 / -1; padding:48px; text-align:center; background:rgba(255,255,255,0.01); border:2px dashed var(--border); border-radius:28px; color:var(--text-3);">
                        <p style="font-weight:700;">No ranks defined yet. Ranks allow e-buddies to showcase their skill level.</p>
                    </div>
                </template>
            </div>
        </div>
