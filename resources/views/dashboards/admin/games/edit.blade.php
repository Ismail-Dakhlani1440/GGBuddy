@extends('layouts.admin', ['title' => 'Edit Game'])
...
<div style="max-width:1000px;">
    ...
    <div style="margin-bottom:32px;">
        ...
        <h1 style="font-size:1.5rem;font-weight:900;letter-spacing:-0.03em;">Edit Game: {{ $game->title }}</h1>
    </div>

    <form action="{{ route('admin.games.update', $game) }}" method="POST" enctype="multipart/form-data" style="display:flex;flex-direction:column;gap:32px;">
        @csrf
        ...
        <div style="display:grid;grid-template-columns:1.5fr 1fr;gap:32px;align-items:start;">
            ...
            <div style="background:var(--surface);border:1px solid var(--border);border-radius:24px;padding:32px;display:flex;flex-direction:column;gap:24px;">
                ...
                <div class="form-group">
                    ...
                    <input type="text" name="title" value="{{ $game->title }}" class="form-input" required placeholder="e.g. Valorant">
                </div>
                
                <div class="form-group">
                    <label class="form-label">Description</label>
                    <textarea name="description" class="form-input" rows="6" placeholder="What is this game about?">{{ $game->description }}</textarea>
                </div>
            </div>
            ...
            <div style="background:var(--surface);border:1px solid var(--border);border-radius:24px;padding:32px;display:flex;flex-direction:column;gap:24px;">
                ...
                <div x-data="{ preview: '{{ $game->cover ? asset('storage/'.$game->cover) : null }}' }" style="display:flex;flex-direction:column;gap:16px;">
                    ...
                    <p style="font-size:12px;color:var(--text-3);">Leave empty to keep current image.</p>
                </div>
            </div>
        </div>

        {{-- Ranks --}}
        <div x-data="{ ranks: @js($game->ranks) }" style="background:var(--surface);border:1px solid var(--border);border-radius:24px;padding:32px;">
                ...
                <template x-for="(rank, index) in ranks" :key="index">
                    <div style="display:flex;gap:12px;align-items:flex-end;padding:16px;background:rgba(255,255,255,0.02);border:1px solid var(--border);border-radius:16px;">
                        ...
                        <div class="form-group" style="flex:3;">
                            ...
                            <input type="text" :name="'ranks['+index+'][title]'" x-model="rank.title" class="form-input" required placeholder="e.g. Diamond IV">
                        </div>
                        ...
                        <template x-if="rank.id">
                             <button type="button" 
                                     @click="if(confirm('Delete this rank?')) { $refs.deleteForm.action = '/admin/ranks/' + rank.id; $refs.deleteForm.submit(); }"
                                     class="btn btn-ghost" style="padding:10px;color:var(--red);border-radius:12px;background:rgba(239,68,68,0.05);border:1px solid rgba(239,68,68,0.1);">
        ...
        <div style="display:flex;gap:16px;padding-top:16px;border-top:1px solid var(--border);">
            <button type="submit" class="btn btn-primary" style="flex:1;padding:16px;font-weight:900;font-size:16px;border-radius:16px;">Update Game</button>
